...
/***
 * Fold Expected is useful for doing monadic computations using expected.
 *
 * Sometimes, when a taking a generic function F : A -> B, and invoking it in
 * a context where failure may occur, you end up making a function that logically
 * looks like,
 *
 ...
 * this problem can be solved by simply making `G` return `return_expected_t<result_of<...>>`.
 *
 * This template is so-named because it plays the same role as the `return` function
 * in a haskell monad, collapsing the doubly-monadic types.
 */

template <typename T>
struct return_expected { typedef expected<T> type; };

template <typename T>
struct return_expected<expected<T>> { typedef expected<T> type; };
