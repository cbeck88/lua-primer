...
push_read() {
  ...
  //`
  //` The standard lua C API provides a number of functions which do things
  //` like,
  //` push a string onto the stack, test if a value on the stack is a string,
  //` fetch a string from the stack. Push an integer onto the stack, test,
  //` fetch,
  ...
  //` Besides primitive types, `push` and `read` can also handle more complex
  //` datatypes.
  //` For instance, a `std::vector` is translated to a table holding a sequence
  //` of values.
  ...
  //` This behavior with `std::vector` requires including the header
  //` `#include <primer/std/vector.hpp>`.
  //`
  //` There are similar extension headers for other kinds of data structures.
  //` (See reference).
  //`
  //` It is also relatively easy to provide overloads for `primer::push` and
  //` `primer::read`.
  //`
  //` These two functions are used by many of the more complex features of
  //` `primer`, so
  //` customizing them will make primer "aware" of your datatypes more or less
  //` everywhere.
...
//` The lua C api provides a function `lua_pushcfunction` which allows you to
//` take a function pointer
//` `int (*)(lua_State *)` and pass it to lua, where scripts can use it like a
//` lua function.

//` When a script calls such a function, your function pointer is called, and
//` the arguments appear on
//` the stack. You have to explicitly use `lua_istring` etc. to test and obtain
//` the arguments, when using the pure C api.
...
//` `primer::read` for your custom
//` data types, then they can be arguments to functions which are adapted this
//` way. See the reference pages for details.
...
//` `userdata` is a special kind of lua value. It cannot be created directly by
//` lua code -- it can only be created by using C API functions.
...
//` them directly.
//` `x` cannot be used with lua functions like `+` that take a number, and
//` `x.id` is an error.
//`
//` An issue with the above code is that if your application has multiple
//` userdata types,
//` then `inspect_token` can lead to undefined behavior -- it uses `static_cast`
//` to convert
//` any userdata type to a `token`.
//`
//` For this and other reasons, you should always set a metatable associated to
//` any userdata value,
//` and use `luaL_testudata` to test that it's the userdata type you think you
//` had.
//`
...
//` In this example, the userdata type `token` is passed by reference to the
//` function `inspect_token`.
//` This implicitly carries out the `luaL_testudata`, and signals an argument
//` error if the it isn't passed
//` a `token`. (i.e. `inspect_token(5)`)
//`
//` Generally userdata must be passed by reference and not by value -- the
//` values exist in lua's memory, not
//` on the stack. It's usually incorrect and undesirable to take a copy.
//`
//` By constrast, when passing `std::string` or `std::vector`, usually it makes
//` the most sense to recieve it
//` by value. It doesn't exist in that format in lua's memory, so primer will
//` make a temporary when it calls
//` your function. You might as well take ownership of that temporary, rather
//` than getting it by `const &`
//` or something.
...
//` able to access the token id, but not change it.
//`
