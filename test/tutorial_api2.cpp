...
// directly, but it can serialize this closure, because it knows how to
// serialize
...
//` That closure object, when called, yields the original `token` userdata
//` value.
//`
...
//`
//` We can serialize the function because `token_ctor` is in the permanents
//` list.
...
//`
//` We can serialize the up-values here because it's just an integer. In
//` general, the
...
//` metatable, and rewrite the above program like so:
