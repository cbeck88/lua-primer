 ...
 *       explicitly take care of userdata metatables created using luaL_newmetatable,
 *       they don't get restored by primer -- this makes sense, as the list of those
...
// Making `metatable` a function taking `lua_State *` means "full manual control"
// for the metatable. We get a lua_State, with the table which will be the metatable
...
//` This is considerably more terse in this case, but in cases of more complex objects,
//` the array syntax may save you from a lot of boiler plate that you are forced
//` to write in the metatable version. Also, when you are in manual control, it's easier
//` to make mistakes. For instance in this case we didn't set a `__gc` metamethod,
//` so in manual control that's a leak, or would be if `token` were not trivial and
//` actually required destruction.

//` Note also that when using literal persistence, we need to put `impl_token_index`
//` in the permanent objects table, or we get an error on persistence. The reason is,
//` when we use literal persistence, eris will also persist the metatable of the
//` userdata type. When we use the closure version, it doesn't actually need to do
//` that, and so `impl_token_index` isn't actually ever encountered when eris traverses
//` the state -- in the closure version, as soon as we encounter the userdata, the
