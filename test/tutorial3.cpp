...
primer::result
new_bar(lua_State * L, const char * s) {
...
//` The above example works because, by default, unless you provide an `__index`
//` function,
//` primer sets __index to refer to the metatable itself -- so `.foo` obtains
//` the function `foo` which we registered.
//` (This is an extremely common technique in lua C api programming. If you
//` really want to have no `__index` function,
//` you should specifiy `nullptr`. See reference for more detail.)
//`
//` Another example, using the token class again, to do something like "operator
//` overloading".
//` Suppose we don't want the `id` value to be directly visible from lua, but we
//` want lua to be able to sort tokens.

//` When using primer, some steps above become unnecessary. Particularly,
//`init_token_metatable` happens implicitly.
//`
//` It's necessary for primer to know how to set up the metatables properly --
//` basically it lets initialization happen
//` lazily, while also allowing serialization and deserialization to happen with
//` less work on your part.

struct token {
  int id;

  bool operator<(const token & other) const { return this->id < other.id; }
...
//` Usually, it is simplest for `metatable` to be a `constexpr std::array` like
//` above.
//` However you have other options, see the reference section for an exhaustive
//` spec.
